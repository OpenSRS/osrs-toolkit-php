...
class openSRS_base
{
     ...
     * @since   3.1
     */
    private function init_socket($host = OSRS_HOST)
    {
        if ($this->is_connected()) {
            return true;
        }
        $this->_socket = fsockopen(CRYPT_TYPE.'://'.$host, OSRS_SSL_PORT, $this->_socketErrorNum, $this->_socketErrorMsg, $this->_socketTimeout);
        if (!$this->_socket) {
            return false;
        } else {
            return true;
